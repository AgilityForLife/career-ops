...
function normalizeStatus(raw) {
  ...
  // Rejected/Rechazado with date → Rejected (strip date)
  if (/^(rejected|rechazado)\s+\d{4}/i.test(s)) return { status: 'Rejected' };

  // Applied/Aplicado with date → Applied (strip date)
  if (/^(applied|aplicado)\s+\d{4}/i.test(s)) return { status: 'Applied' };
  ...
  if (['enviada', 'aplicada', 'aplicado', 'sent'].includes(lower)) return { status: 'Applied' };
